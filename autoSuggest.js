...
const allMonNames = Object.keys(genDex);

function findSuggestions(strInputId, strSuggestionsDivId, suggestionArray) { 
...
function autocomplete(inp) {
    ...
    inp.addEventListener("input", function(e) {
        ...
        for (i = 0; i < allMonNames.length; i++) {
          /*check if the item starts with the same letters as the text field value:*/
          if (allMonNames[i].substr(0, val.length).toUpperCase() == val.toUpperCase()) {
            /*create a DIV element for each matching element:*/
            b = document.createElement("DIV");
            /*make the matching letters bold:*/
            b.innerHTML = "<strong>" + allMonNames[i].substr(0, val.length) + "</strong>";
            b.innerHTML += allMonNames[i].substr(val.length);
            /*insert a input field that will hold the current array item's value:*/
            b.innerHTML += "<input type='hidden' value='" + allMonNames[i] + "'>";
